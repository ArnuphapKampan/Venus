...
import './contract.css';
import PartOne from './formContractEdit/1-partOne';
import PartTwo from './formContractEdit/2-partTwo';
import PartThree from './formContractEdit/3-partThree';
import PartFour from './formContractEdit/4-partFour';
import PartFive from './formContractEdit/5-partFive';
import PartSix from './formContractEdit/6-partSix';
import PartInstallment from './formContractEdit/partInstallment';
import PartSeven from './formContractEdit/7-partSeven';
import PartEight from './formContractEdit/8-partEight';
import PartNine from './formContractEdit/9-partNine';
import PartTen from './formContractEdit/10-partTen';
import PartEleven from './formContractEdit/11-partEleven';
import PartTwelve from './formContractEdit/12-partTwelve';
...
const ContractEdit = () => {
    ...
    const loadContractInfo = () =>{
      handlerGetInfoEditContract(contractID,authtoken).then( (res) => {

          const detail = JSON.parse(res.data[0].detail)
          const section_1 = JSON.parse(res.data[0].section_1)
          const section_2 = JSON.parse(res.data[0].section_2)
          const section_3 = JSON.parse(res.data[0].section_3)
          const section_4 = JSON.parse(res.data[0].section_4)
          const installmentArray = JSON.parse(res.data[0].installment)
        //Detail
          setDate(detail.date);
          ...
          document.getElementsByName('prefix')[1].value = detail.prefix
          document.getElementsByName('employer_name')[1].value = detail.employer_name
          document.getElementsByName('employer_surname')[1].value = detail.employer_surname
          document.getElementsByName('employer_age')[1].value = detail.employer_age
          document.getElementsByName('employer_personal_code')[1].value = detail.employer_personal_code
          document.getElementsByName('employer_house_number')[1].value = detail.employer_house_number
          document.getElementsByName('employer_village_group')[1].value = detail.employer_village_group
          document.getElementsByName('employer_alley')[1].value = detail.employer_alley
          document.getElementsByName('employer_road')[1].value = detail.employer_road
          document.getElementsByName('employer_sub_district')[1].value = detail.employer_sub_district
          document.getElementsByName('employer_district')[1].value = detail.employer_district
          document.getElementsByName('employer_province')[1].value = detail.employer_province
          document.getElementsByName('employer_zip_code')[1].value = detail.employer_zip_code

          document.getElementsByName('contractor_company')[1].value = detail.contractor_company
          document.getElementsByName('contractor_company_number')[1].value = detail.contractor_company_number
          document.getElementsByName('contractor_company_by')[1].value = detail.contractor_company_by
          document.getElementsByName('contractor_office_number')[1].value = detail.contractor_office_number
          document.getElementsByName('contractor_village_group')[1].value = detail.contractor_village_group
          document.getElementsByName('contractor_alley')[1].value = detail.contractor_alley
          document.getElementsByName('contractor_road')[1].value = detail.contractor_road
          document.getElementsByName('contractor_sub_district')[1].value = detail.contractor_sub_district
          document.getElementsByName('contractor_district')[1].value = detail.contractor_district
          document.getElementsByName('contractor_province')[1].value = detail.contractor_province
          document.getElementsByName('contractor_zip_code')[1].value = detail.contractor_zip_code
        //Section 1
          document.getElementsByName('section1_residential_house')[1].value = section_1.section1_residential_house
          document.getElementsByName('section1_deed_no')[1].value = section_1.section1_deed_no
          document.getElementsByName('section1_area')[1].value = section_1.section1_area
          document.getElementsByName('section1_land_number')[1].value = section_1.section1_land_number
          document.getElementsByName('section1_page_area')[1].value = section_1.section1_page_area
          document.getElementsByName('section1_sub_district')[1].value = section_1.section1_sub_district
          document.getElementsByName('section1_district')[1].value = section_1.section1_district
          document.getElementsByName('section1_province')[1].value = section_1.section1_province
          document.getElementsByName('section1_ri')[1].value = section_1.section1_ri
          document.getElementsByName('section1_ngan')[1].value = section_1.section1_ngan
          document.getElementsByName('section1_tarang_wa')[1].value = section_1.section1_tarang_wa
        //Section 2
          document.getElementsByName('section2_assign_agency')[1].value = section_2.section2_assign_agency
          document.getElementsByName('section2_assign_contract')[1].value = section_2.section2_assign_contract
        //Section 3
          document.getElementsByName('section3_wage_total')[1].value = section_3.section3_wage_total
          document.getElementsByName('section3_wage_percent')[1].value = section_3.section3_wage_percent
          document.getElementsByName('section3_amount')[1].value = section_3.section3_amount
          document.getElementsByName('section3_before_amount')[1].value = section_3.section3_before_amount
          document.getElementsByName('section3_outstanding_percent')[1].value = section_3.section3_outstanding_percent
          document.getElementsByName('section3_outstanding_amount')[1].value = section_3.section3_outstanding_amount
          document.getElementsByName('section3_balance')[1].value = section_3.section3_balance
          document.getElementsByName('section3_installment_amount')[1].value = section_3.section3_installment_amount
        //Section 4
          document.getElementsByName('section4_expense_amount')[1].value = section_4.section4_expense_amount
        //Installment
          setInstallmentAmount(installmentArray.length);
          setDateInstallment(installmentArray);
      })
    }
    ...
    const onSubmitData = async (e) => {
      ...
      const detail = {
       ...
       updateContract(detail,idLoading);
    }

    const updateContract = (detail,idLoading) => {
      const URL    =  document.URL;
      const status = "edit";
      const engine = "edit contract";
      const info = {
        id:contractID,
        detail:detail,
        url: URL,
      ...
      updateContractHandler(info,authtoken).then(res =>{
        setTimeout(function(){
    ...
    const mockup = {
      ...
      installmentStatusENG:undefined,
      installmentStatusTH:undefined,
      payment:undefined,
      startDate:undefined,
      stopDate:undefined
    }

    if(installmentAmount > dateInstallment.length){
      let length = installmentAmount - dateInstallment.length;
      for (let i = 0; i < length; i++) {
        dateInstallment.push(mockup)
      }
      for (let i = 0; i < installmentAmount; i++) {
        installment.push(<PartInstallment key={i} installment={i+1} final={installmentAmount} info={dateInstallment[i]} />);
      }
    }else{
      for (let i = 0; i < installmentAmount; i++) {
        installment.push(<PartInstallment key={i} installment={i+1} final={installmentAmount} info={dateInstallment[i]} />);
